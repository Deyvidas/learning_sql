...
- [Группировка записей таблицы используя `GROUP BY`](#группировка-записей-таблицы-используя-group-by)
- [Как работает `COUNT`](#как-работает-count)
- [Использование групповых функции `MIN` `MAX` `AVG` `SUM`](#использование-групповых-функции-min-max-avg-sum)
- [Группировка данных по условию `HAVING`](#группировка-данных-по-условию-having)
---
## Группировка записей таблицы используя `GROUP BY`
...
```sql
SELECT
    <field_1>  -- fields here must be equal with fields after GROUP BY;
FROM
    <tbl_name>
WHERE
    <condition_1>
GROUP BY
    <field_1>  -- fields here must be equal with fields after SELECT;
;
```
Пример 1:
```sql
